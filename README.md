...
3. `Backend` преобразовывает `AST` в текстовый файл, являющийся `ассемблерным кодом` x86-64 на диалекте `NASM`. _(Подробнее см. ниже)_
...
Расположена в `tomato_stdlib`, частично написана на ассемблере, частично на Си. Не использует стандартную библиотеку Си ценой очень простых и скудных реализаций ф-й печати и ввода числа (что, впрочем, под стать духу языка). Реализованы следующие основные функции:
