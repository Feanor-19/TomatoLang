...
*/
static DriverConfig assemble_config(size_t n_flags, CmdLineFlag flags[]);

//----------------------------------------------------------------

DriverConfig get_config(int argc, const char *argv[])
{
    assert(argc > 0);
    assert(argv != NULL);
    ...
    DriverConfig cfg = assemble_config(N_FLAGS, supported_flags);
...
static DriverConfig assemble_config(size_t n_flags, CmdLineFlag flags[])
{
    ...
    DriverConfig config = {"", "", "", "", CONFIG_NO_ERROR};
...
void print_config(FILE *stream, DriverConfig cfg)
{
    assert(stream != NULL);
...
void print_cfg_error_message(FILE *stream, DriverConfigError error)
{
    assert(stream != NULL);
